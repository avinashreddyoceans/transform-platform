...
class TransformMetrics(private val registry: MeterRegistry) {
    ...
    fun fileSubmitted(specId: String) = registry.counter("transform.files.submitted", "specId", specId).increment()
    ...
    fun windowEventCollected(profileId: String) = registry.counter("window.events.collected", "profileId", profileId).increment()

    fun windowEventDuplicate(profileId: String) = registry.counter("window.events.duplicates", "profileId", profileId).increment()

    fun windowOpened(profileId: String) = registry.counter("window.open.count", "profileId", profileId).increment()
    ...
    fun integrationFileDelivered(integrationId: String, channelType: String, status: String) = registry.counter(
        "integration.files.delivered",
        "integrationId",
        integrationId,
        "channelType",
        channelType,
    ...
    private fun fileDurationTimer(specId: String, status: String): Timer = Timer.builder("transform.file.duration")
    ...
    private fun windowDurationTimer(profileId: String, status: String): Timer = Timer.builder("window.lifecycle.duration")

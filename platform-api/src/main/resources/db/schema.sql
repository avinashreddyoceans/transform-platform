...
CREATE INDEX idx_windows_pending_open
    ON windows (scheduled_open_at)
    WHERE status = 'PENDING';

CREATE INDEX idx_windows_open_close
...
CREATE INDEX idx_windows_updated        ON windows (updated_at DESC);
...
CREATE INDEX idx_fl_direction    ON file_log (direction);
CREATE INDEX idx_fl_status       ON file_log (status);
CREATE INDEX idx_fl_window       ON file_log (window_id);
CREATE INDEX idx_fl_profile      ON file_log (profile_id);
CREATE INDEX idx_fl_integration  ON file_log (integration_id);
CREATE INDEX idx_fl_arrived      ON file_log (arrived_at DESC);
CREATE INDEX idx_fl_execution    ON file_log (generated_by_execution_id);
...
CREATE UNIQUE INDEX idx_fl_dedup_inbound
...
CREATE INDEX idx_wevt_window_type  ON window_events (window_id, record_type);
CREATE INDEX idx_wevt_profile_time ON window_events (profile_id, arrived_at DESC);
CREATE INDEX idx_wevt_client_time  ON window_events (client_id, arrived_at DESC);

CREATE UNIQUE INDEX idx_wevt_dedup
...
CREATE INDEX idx_fspec_format  ON file_specs (format);
CREATE INDEX idx_fspec_updated ON file_specs (updated_at DESC);
...
CREATE TABLE window_action_executions (
...
CREATE INDEX idx_wae_active
    ON window_action_executions (window_id)
    WHERE status IN ('PENDING', 'RUNNING', 'RETRYING');

CREATE INDEX idx_wae_status       ON window_action_executions (status);
CREATE INDEX idx_wae_window       ON window_action_executions (window_id);
CREATE INDEX idx_wae_profile      ON window_action_executions (profile_id);
CREATE INDEX idx_wae_last_updated ON window_action_executions (last_updated_at DESC);
...
CREATE INDEX idx_stepexec_execution ON workflow_step_executions (execution_id);
CREATE INDEX idx_stepexec_step      ON workflow_step_executions (execution_id, step_id);
CREATE INDEX idx_stepexec_status    ON workflow_step_executions (status);
